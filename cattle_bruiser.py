...
#lock variables so they dont error during concurrency, releases after variables are updated
...
def main():
    ...
    try:
        ...
        procs = []
        if concurrent_cpu >= 1:
            p1 = multiprocessing.Process(target = start_request(num_of_atks, site))
            procs.append(p1)
            count += 1
          #  p1.start()
            if concurrent_cpu >= 2:
                p2 = multiprocessing.Process(target = start_request(num_of_atks, site))
                procs.append(p2)
                count += 1
           #     p2.start()
                if concurrent_cpu >= 3:
                    p3 = multiprocessing.Process(target = start_request(num_of_atks, site))
                    procs.append(p3)
                    count += 1
            #        p3.start()
                    if concurrent_cpu == 4:
                        p4 = multiprocessing.Process(target = start_request(num_of_atks, site))
                        procs.append(p4)
                        count += 1
             #           p4.start()
        if count == 1:
